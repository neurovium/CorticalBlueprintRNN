...
The diagnostics that justify this choice are produced by `6_6_2_sttc_corr_metric_dist_check.ipynb` (see [03 — Distribution diagnostics](03-Distribution-Diagnostics.md)).

## Notebooks at a glance

| notebook | input | what it computes | paper artifact |
|---|---|---|---|
| `6_6_2_sttc_corr_metric_dist_check.ipynb` | all 8 `*_raw.csv` files for one variant (default `W*D*C`) across all 3 tasks | per-metric distribution plots, mutual-information ranking between metrics, Spearman correlation heatmap, KW omnibus, MWU pairwise | motivates nonparametric choice |
| `6_6_2_confidence_intervals.ipynb` | one `<metric>_raw.csv` per task across all 11 variants | 95% CIs per variant per task (t-distribution + bootstrap) | parenthesized CIs in Table 2 (& Table 3) |
| `The_Effect_of_model_variants_Task_1.ipynb` | all 8 `*_raw.csv` files for Task 1 | KW + MWU + Holm per factor ($W$, $D$, $C$); robustness arms | Supplementary Tables S5/S6/S7, Task 1 |
| `The_Effect_of_model_variants_Task_2.ipynb` | Task 2 equivalent | same | S5/S6/S7, Task 2 |
| `The_Effect_of_model_variants_Task_3.ipynb` | Task 3 equivalent | same | S5/S6/S7, Task 3 |
